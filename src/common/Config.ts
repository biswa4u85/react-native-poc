export default {
  // Localhost
  // apiSocketUrl: 'http://localhost:3008',
  // apiBaseUrl: 'http://localhost:3008/api/v1',
  // uploadPath: 'http://localhost:3008/LaundryPlusImages',
  // groupIds: {
  //   admins: '5df90da55c69b6402838d297',
  //   users: '5df90db05c69b6402838d298',
  //   vendors: '5df90db95c69b6402838d299',
  //   deliveryboys: '5df90dc25c69b6402838d29a',
  // },
  apiSocketUrl: 'http://moclass.in:3008',
  apiBaseUrl: 'http://moclass.in:3008/api/v1',
  uploadPath: 'http://moclass.in/LaundryPlusImages',
  groupIds: {
    superadmin: '5ed1ed6dae6acd0278cc4b83',
    admins: '5ed1ed8dae6acd0278cc4b85',
    users: '5ed1ed98ae6acd0278cc4b86',
    vendors: '5ed1eda6ae6acd0278cc4b87',
    deliveryboys: '5ed1edb1ae6acd0278cc4b88',
  },
  showStatusBar: true,
  title: 'iLaundry',
  googleMap: 'AIzaSyB_zZZJR2C867GHH4jBbNs0IDsWX8lKMAw',
  messageStatus: {
    0: 'New Orders',
    1: 'Waiting For Customer',
    2: 'Pickup Orders',
    3: 'Work Progress',
    4: 'Drop Orders',
    5: 'Complete Orders',
    6: 'Cancel Orders',
    AssignToPickup: 'Waiting for Pickup',
    AcceptByPickup: 'Accept From Customer',
    AcceptFromCustomer: 'Delivery To Vender',
    AssignToDrop: 'Waiting for Drop',
    AcceptByDrop: 'Accept From Vender',
    AcceptFromVender: 'Delivery To Customer',
    CompleteOrder: 'Complete Order',
    CancelOrder: 'Cancel By ',
    NewOrder: 'New Order',
    DeliveryToVendors: 'Delivery To Vendors',
    DeliveryToCustomer: 'Delivery To Customer',
    Vender: 'Vender',
  },
}